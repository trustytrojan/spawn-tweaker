...
public class OnJoinConfig
{
	...
	{
		this.minHealthChance = (minHealthChance != null) ? new LinkedHashMap<>(minHealthChance) : new LinkedHashMap<>();
	}
	...
	public boolean shouldAllowJoin(final EntityLiving el, final Random rand)
	{
		...
		try
		{
			if (el.world != null)
			{
				...
				{
					logger.debug("on_join: entity {} allowed due to chunk generation (chunk not populated yet)",
						el.getName());
					return true;
				}
			}
		...
		{
			logger.warn("on_join: could not determine chunk population state for entity {}: {}", el.getName(),
				th.getMessage());
		}
		...
		if (chance != null)
		{
			...
			logger.info("on_join health_check: entity=\"{}\", maxHealth={}, chance={}, roll={:.2f}, allow={}",
				el.getName(), health, chance, roll, allow);
			return allow;
		}
